...
type UsersService interface {
	SignUp(ctx context.Context, req request.UsersCreate) response.Users
	SignIn(ctx context.Context, username string, password string) response.Users
	FindById(ctx context.Context, usersId int) response.Users
	Update(ctx context.Context, req request.UsersUpdate) response.Users
...
func (services *UsersImpl) SignUp(ctx context.Context, req request.UsersCreate) response.Users {
...
func (services *UsersImpl) SignIn(ctx context.Context, username string, password string) response.Users {
	tx, err := services.DB.Begin()
	helper.PanicIfError(err)
	defer helper.CommitOrRollback(tx)

	user, err := services.UsersRepository.SignIn(ctx, tx, username, password)
	if err != nil {
...
func (services *UsersImpl) Update(ctx context.Context, req request.UsersUpdate) response.Users {
	err := services.Validate.Struct(req)
	helper.PanicIfError(err)
